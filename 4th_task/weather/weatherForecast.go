...
func (wf *WeatherForecast) FormatWeather(w Weather) string {
	//Используемые переменные для вывода итогового сообщения
	...
	//Проверка для вставки "+" в строку со значением температуры
	if t>0{
		temp="+"+strconv.FormatFloat(t, 'f', -1, 32)
	}
	//Проверка для вставки порывов
	if wGust!=0{
		gust=" с порывами до "+strconv.Itoa(wGust)+"м/с"
	}

	return "Сегодня в городе " + w.GetCity() + " " + w.GetCloudiness() +
